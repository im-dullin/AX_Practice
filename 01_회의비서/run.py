#!/usr/bin/env python3
"""
회의 비서 — 녹음/음성/텍스트 → HTML 리포트 자동 오픈

Usage:
  python run.py --record                       # 마이크 녹음 (Enter로 종료) → 전사 → 요약
  python run.py samples/meeting.m4a            # 기존 오디오 파일
  python run.py samples/sample_transcript.txt  # 텍스트 파일 (즉시 데모용)
"""
import sys
import json
import re
...
def transcribe(src: Path) -> str:
    """텍스트면 그대로, 오디오면 faster-whisper로 전사."""
    ...
    segments, _info = model.transcribe(str(src), language="ko", vad_filter=True)
    return "\n".join(s.text.strip() for s in segments if s.text.strip())


def summarize(transcript: str) -> dict:
    """Claude Code CLI로 요약 + 액션 추출."""
...
def main():
    ...
    print(f"📥 입력: {src.name}")
    print("📝 전사..." if src.suffix.lower() not in {".txt", ".md"} else "📄 텍스트 로드...")
